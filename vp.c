...
#include "misc.h"
// ./misc.h
// ./simplescalar/sslittle-na-sstrix/include/misc.h
// ./simplescalar/ssbig-na-sstrix/include/misc.h

// #include "ss.h"
// ./simplescalar/gcc-2.7.2.3/config/ss/ss.h
// ./simplescalar/simpleutils-990811/include/coff/ss.h
// ./simplescalar/simpleutils-990811/include/opcode/ss.h
...
// SS_TIME_TYPE              sim_cycle;        /* simulation time */
...
int                       use_vp;           /* use value prediction */
...
int                       hash_asso_i2;     /* Double precision set
					       associativity */
int                       predicted_ok;     /* total number of correct
					       predictions */
...
// int update(
//  SS_ADDR_TYPE instpc, /* The instruction address */
//  SS_INST_TYPE inst,   /* The instruction opcode and registers */
...
//   flag[0]=0;
//   flag[1]=0;
//   op=SS_OPCODE(inst);  /* recover opcode from inst */
//   if(SS_OP_FLAGS(op)&F_SINGLE_P){
//     /* single precision output */
//      val[0]=calc_val.value.single_p.i;
...
// 	return(MISS);
//      else{
//         if(val[0]==pred_val.value.single_p.i){
//               /*hit*/
...
//      val[0]=calc_val.value.double_p.ii[0];
//      val[1]=calc_val.value.double_p.ii[1];
...
//      else{
// 	for(j=0;j<2;j++)
//          if(val[j]==pred_val.value.double_p.ii[j]){
//             /*hit*/
...
// void allocate(SS_ADDR_TYPE pred_PC, /* allocated inst's address */
// SS_INST_TYPE inst, /* allocated inst's opcode and registers */
...
// enum ss_opcode op;     /* instruction opcode */
//
...
//     */
// void lookup(SS_ADDR_TYPE pred_PC,/* The instruction address */
// SS_INST_TYPE inst, /* The instruction calculated value and flags */
// VAL_TAG_TYPE *pred_val   /* The instruction predicted value and flags */)
// {
...
//   flag[1]=0;
//   op=SS_OPCODE(inst); /* recover op code from inst */
//
//   if(SS_OP_FLAGS(op)&F_SINGLE_P){
//     /* single precision */
//      index = (unsigned int)(pc % hash_no_i1);
...
// 	pred_val->value.single_p.i=(hash_table_i1[index][i].last_val+((use_stride==0)?0:val[0]));	/*hit*/
// 	/* fsm classification:
...
//            pred_val->value.double_p.ii[0]=(hash_table_i2[index][i].last_val[0]+((use_stride==0)?0:val[0]));      /*hit*/
//            pred_val->value.double_p.ii[1]=(hash_table_i2[index][i].last_val[1]+((use_stride==0)?0:val[1]));     /*hit*/
